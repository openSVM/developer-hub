...
#### `MintLimit` Accounts
When de [UWUIFY_TOKEN_1744632705115_39](/core-candy-machine/guards/mint-limit) guawd is active, it's advisabwe to wetwieve de `MintCounter` account fow de usew's wawwet~ Dis awwows you to check whedew de usew has weached deiw minting wimit ow if dey'we stiww ewigibwe to mint additionyaw items.
...
Simiwaw to de `MintLimit` guawd it can make sense to fetch de `NftMintCounter` account of de [UWUIFY_TOKEN_1744632705115_45](/core-candy-machine/guards/nft-mint-limit) guawd to vewify ewigibiwity.
...
Simiwaw to de `NftMintCounter` guawd it can make sense to fetch de `AssetMintCounter` account of de [UWUIFY_TOKEN_1744632705115_50](/core-candy-machine/guards/asset-mint-limit) guawd to vewify ewigibiwity.
...
If you add to many `mintV1` instwuctions into a twansaction you wiww weceive a `Transaction too large` ewwow~ De function [UWUIFY_TOKEN_1744632705115_102](/umi/transactions#transaction-builders) awwows to check fow dis befowe sending de twansaction so dat de twansaction can be spwit befowe being sent~ In case spwitting is nyeeded using [UWUIFY_TOKEN_1744632705115_103](/umi/transactions#building-and-signing-transactions) is wecommended so dat onwy onye popup has to be appwuvd in de Wawwet Adaptew~    
